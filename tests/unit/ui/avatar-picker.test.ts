...
describe("avatar customization", () => {
  it("restores a combination without calling back before construction finishes", () => {
    ...
    expect(document.querySelectorAll(".char-tile[data-character]")).toHaveLength(23)
  ...
  it("restores Morinji's theme and clears it when the face is claimed or changed", () => {
    localStorage.setItem("role-room:last-character", "morinji")
    const picker = buildCharacterPicker("en", [], vi.fn())
    document.body.append(picker.element)
    expect(document.documentElement.dataset.characterTheme).toBe("morinji")
    picker.setTaken(["morinji"])
    expect(document.documentElement.dataset.characterTheme).toBeUndefined()
    picker.setTaken(CHARACTERS.filter(c => c.id !== "morinji").map(c => c.id))
    click(".avatar-surprise")
    expect(document.documentElement.dataset.characterTheme).toBe("morinji")
    expect(picker.accessory()).toBe("")
    picker.setTaken([])
