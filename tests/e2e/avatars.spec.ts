...
test("mobile avatar studio has 21 faces, removable accessories and usable tabs", async ({ browser }) => {
  const hostContext = await browser.newContext()
  const host = await hostContext.newPage()
  ...
  await expect(page.locator(".char-tile[data-character]")).toHaveCount(21)
